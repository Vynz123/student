...
1.1.1.1.1 - No an IP adress compises four segments separated by periods, not five.
23.23.23.23 - Yes this is a valid IP address as it follows the correct format of four segments.
134.492.100.0 - No as each segment of an IP address should be within the range of 0 to 255.
255.256.55.255 - N the max val for each seg in an IP address is 255.
2.93.255.19 - Y this is a possible it complies with the four-segment format within the valid range.
...
Multicast - No multicast refers to a message sent to a specific group of computers, not every computer on the network.
Unicast - No unicast involves sending a message from one sender to one specific recipient.
Broadcast - Yes this is an example of broadcasting where a message is sent to all devices on a particular network without specifying any individual recipient.
Models
...
ASCII - It's not a protocol rather it's a character encoding scheme used across multiple layers for data representation.
FTP - This operates at the Application layer (Layer 7) as it deals with file transfer services.
TLS - It operates at the Transport layer (Layer 4) to ensure secure communication.
USB - This operates at the Physical layer (Layer 1) as it's primarily concerned with the physical connection between devices.
Telnet is an internet protocol that allows remote access to other computers over a local network or the internet. What layer of the OSI model would this protocol be located on? What is the function of this layer?
Telnet would be situated at the Application layer (Layer 7) of the OSI model. This layer provides network services to applications and end-user processes, facilitating user interfaces and file transfers between networked devices.
DNS
...
Bob should buy the "best.com" domain from a DNS provider. The subdomains would be "is.the" and "bob" accordingly.
HTTP and HTTPS
What is a difference between HTTP and HTTPS?
HTTPS uses SSL/TLS (Secure Socket Layer/Transport Layer Security) for encryption, while HTTP doesn't employ such encryption protocols.
Last trimester we sent HTTP requests for our passion projects
Did we use HTTP or HTTPS?
We used HTTP.
Benefits: Faster data transfer, simplicity.
Disadvantages: Lack of security, vulnerability to interception.
TCP and UDP
Bob is setting up a video streaming service, and he needs the stream to be real-time.
He should use UDP for its lower latency and suitability for real-time applications.
Cons include lack of error checking, potential packet loss. For instance, video freezing or stuttering due to lost packets.
TCP has error checking, which ensures that all packets arrive properly. Why is this important?
TCP's error checking is crucial for reliable data transmission, ensuring that all packets reach the destination without corruption or loss.
Example: If a segment fails checksum verification, TCP requests retransmission of that segment.
Server A computer is communicating with Server B.
In TCP, Server B acknowledges receipt of packets by sending acknowledgment messages back to Server A. In UDP, no such acknowledgment mechanism exists.
Another use: Ensuring data integrity and maintaining synchronization between communicating devices.